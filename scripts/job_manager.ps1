...
class PSJobManager {
    ...
    Removes all jobs in the Topic and finally removes the tracker itself.
    #>
    [void] RemoveTopic ($Topic) {
        Write-Host "Removing jobs from tracker: $Topic"
    ...
    Waits for all jobs in a tracker to be in non-Running state.
    #>
    ...
    [Array] GetJobOutputs ($Topic) {
        Write-Host "Retriving Output from Jobs in Topic: $Topic"
        [Console]::InputEncoding = New-Object Text.UTF8Encoding $false 
        $results = @()
        $current = $this.Jobs[$Topic].First
        while(-not ($current -eq $null)) {
            $results += $current.Value | Receive-Job -Keep
            $current = $current.Next
        }
        return $results
    }
